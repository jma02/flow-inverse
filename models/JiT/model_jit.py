...
class JiTBlock(nn.Module):
    ...
    @torch.compile
    def forward(self, x, c, context, feat_rope=None):
...
class JiT(nn.Module):
    ...
    def __init__(
        ...
        # time and conditioning image embed
        ...
        # linear embed
        x_embed_in_channels = in_channels + cond_in_channels
        self.x_embedder = BottleneckPatchEmbed(input_size, patch_size, x_embed_in_channels, bottleneck_dim, hidden_size, bias=True)
    ...
    def initialize_weights(self):
        ...
        # Initialize conditioning patch embedder:
        w1 = self.cond_embedder.proj1.weight.data
        nn.init.xavier_uniform_(w1.view([w1.shape[0], -1]))
        w2 = self.cond_embedder.proj2.weight.data
        nn.init.xavier_uniform_(w2.view([w2.shape[0], -1]))
        nn.init.constant_(self.cond_embedder.proj2.bias, 0)
    ...
    def forward(self, x, t, cond):
        ...
        cond: (N, Cc, H, W)
        """
        # conditioning image and time embeddings
        t_emb = self.t_embedder(t)
        cond_tokens = self.cond_embedder(cond)
        cond_emb = cond_tokens.mean(dim=1)
        c = t_emb + cond_emb

        # forward JiT (pooled conditioning)
        ...
        for i, block in enumerate(self.blocks):
            ...
            x = block(x, c, context, self.feat_rope if i < self.in_context_start else self.feat_rope_incontext)
...
def JiT_XS_8(**kwargs):
    return JiT(depth=8, hidden_size=512, num_heads=8,
