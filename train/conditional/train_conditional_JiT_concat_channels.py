...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='circles-eit-cond-dtn-concat-JiT', help='Directory to save models')
    ...
    model = JiTUncond_B_8(in_channels=2, out_channels=1, input_size=128).to(device)
        ...
        with torch.no_grad():
            ...
            def v_field(z, t):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    z_uncond = torch.cat([z, torch.zeros_like(x_cond)], dim=1)
                    z_cond = torch.cat([z, x_cond], dim=1)
                    v_uncond = eval_model(z_uncond, t_batch)
