...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train a JiT flow matching model for v prediction (DTN).")
    parser.add_argument('--device', type=str, default='cuda:0', help='Device to use for training')
    parser.add_argument('--ckpt', type=str, default=None, help='Path to a checkpoint to resume training from')
    parser.add_argument('--save_dir', type=str, default='circles-eit-v-pred-dtn-jit', help='Directory to save models')
        ...
        for i, (x,) in tqdm(enumerate(train_loader), desc=f"Epoch {epoch}", leave=False):
            ...
            if (log_step + 1) % log_freq == 0:
                pbar.set_postfix_str(
                    f'Step: {log_step} ({epoch}) | Loss: {true_loss:.5f} | Grad: {grad.item():.5f}'
                )
