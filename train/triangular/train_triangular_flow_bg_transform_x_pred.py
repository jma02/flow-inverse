...
from models.triangular import UnetTriangular
import argparse
...
def get_loss_fn(model: UnetTriangular):
    def loss_fn(x: Tensor) -> Tensor:
        ...
        z_t = interpolate_xt(t, x0, x1)
        denom = torch.clip(1 - t_broadcast, min=5e-2)
        v = (x1 - z_t) / denom
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='circles-eit-triangular-flow-default-bg-transform-x-pred', help='Directory to save models')
    ...
    mat_contents = sio.loadmat(mesh_file)
    p = torch.tensor(mat_contents['p'], dtype=torch.float64).to(device)
    t_mesh = torch.tensor(mat_contents['t']-1, dtype=torch.long).to(device)
    vol_idx = torch.tensor(mat_contents['vol_idx'].reshape((-1,))-1, dtype=torch.long).to(device)
    bdy_idx = torch.tensor(mat_contents['bdy_idx'].reshape((-1,))-1, dtype=torch.long).to(device)
    mesh = Mesh(p, t_mesh, bdy_idx, vol_idx)
    v_h = V_h(mesh)
    background = dtn_from_sigma(
    ...
    model = UnetTriangular(unet_ch=32, y_ch=1, theta_ch=1, guidance_scale=1.0).to(device)
    ...
    for epoch in pbar:
        ...
        with torch.no_grad():
            ...
            train_X_min_dev = train_X_min.to(device)
            train_X_max_dev = train_X_max.to(device)
            train_Y_min_dev = train_Y_min.to(device)
            train_Y_max_dev = train_Y_max.to(device)

            def v_field(z, t):
                t_batch = t.expand(z.shape[0])
                t_broadcast = t_batch[:, None, None, None]
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    x_pred = model(z, t_batch, guidance_scale=SAMPLE_GUIDANCE_SCALE)
                denom = torch.clip(1 - t_broadcast, min=5e-2)
            ...
            x_gen_div_bg = 0.5 * (x_gen + 1.0) * (train_X_max_dev - train_X_min_dev) + train_X_min_dev
            y_gen_phys = 0.5 * (y_gen + 1.0) * (train_Y_max_dev - train_Y_min_dev) + train_Y_min_dev
