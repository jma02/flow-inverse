...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='circles-eit-inverse-unet-interp-y-pred-default-bg-transform', help='Directory to save models')
    ...
    for epoch in pbar:
        ...
        with torch.no_grad():
            ...
            x = x_batch[0].unsqueeze(0).to(device)
            y = y_batch[0].unsqueeze(0).to(device)
            ...
            for level in noise_levels:
                ...
                t = torch.full((x.shape[0],), level, device=device)
                x_noisy = interpolate_xt(t, noise, x)
                output = model(x_noisy, t)

                x_vis = 0.5 * (x_noisy.squeeze() + 1.0) * (train_X_max - train_X_min) + train_X_min
                ...
                sweep_results.append({
                    ...
                    "x_vis": x_vis,
                    "out_vis": out_vis,
                    "err_vis": np.abs(out_vis - y_vis),
                })

            vmin = float(min(result["out_vis"].min() for result in sweep_results + [{"out_vis": y_vis}]))
            vmax = float(max(result["out_vis"].max() for result in sweep_results + [{"out_vis": y_vis}]))
            ...
            column_titles = ["Noisy DtN", "UNet Output", "Ground Truth", "Abs Error"]
            for col, title in enumerate(column_titles):
                axs[0, col].set_title(title, fontdict=title_font)

            for row_idx, result in enumerate(sweep_results):
                axs[row_idx, 0].imshow(result["x_vis"], cmap="magma")
                axs[row_idx, 0].set_ylabel(f"t = {result['level']:.2f}", fontdict=title_font, rotation=90)

                axs[row_idx, 1].imshow(result["out_vis"], cmap="Blues", vmin=vmin, vmax=vmax)
                axs[row_idx, 2].imshow(y_vis, cmap="Blues", vmin=vmin, vmax=vmax)
                axs[row_idx, 3].imshow(result["err_vis"], cmap="inferno")
